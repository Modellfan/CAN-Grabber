...
Source files:
- `src/dev/upload_ui_test_v5/main.cpp`
- `src/dev/upload_ui_test_v5/storage.cpp`
- `src/dev/upload_ui_test_v5/uploader.cpp`
- `src/dev/upload_ui_test_v5/webserver.cpp`
- `src/dev/upload_ui_test_v5/serial_tester_interface.cpp`

## 2) Runtime Components
| Component | File | Trigger/Loop | Responsibility |
|---|---|---|---|
| Boot + task wiring (`setup`) | `src/dev/upload_ui_test_v5/main.cpp` | One-shot at boot | Parse target URL, connect Wi-Fi, init SD, seed queue, create tasks |
| Serial polling (`loop`) | `src/dev/upload_ui_test_v5/main.cpp` | Arduino main loop | Dispatch non-blocking serial commands |
| Upload Manager Task (`upload_task`) | `src/dev/upload_ui_test_v5/uploader.cpp` | FreeRTOS loop | Pop queued file, upload multipart HTTP, apply retry policy, update stats, mark uploaded-state |
| HTTP Server Task (`server_task`) | `src/dev/upload_ui_test_v5/webserver.cpp` | FreeRTOS loop | Run `WebServer::handleClient()`, serve UI/API endpoints |
| Monitor Task (`monitor_task`) | `src/dev/upload_ui_test_v5/uploader.cpp` | FreeRTOS loop (1 s) | Wi-Fi reconnect check, periodic pending-file scan, periodic progress logs |
| Prefetch Task (`prefetch_task`) | `src/dev/upload_ui_test_v5/uploader.cpp` | FreeRTOS task | Currently disabled (immediate self-delete) |
| Shared state/storage helpers | `src/dev/upload_ui_test_v5/storage.cpp` | Called by tasks/endpoints | Queue ops, stats snapshots, probe cache/stats, SD and uploaded-state helpers |
