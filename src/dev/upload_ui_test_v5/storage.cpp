...
// Read the current reachability probe stats atomically.
...
// Update probe stats after each reachability attempt.
...
// Resolve configured target host without DNS cache.
...
// Resolve target host using short-lived cached IP fallback path.
...
// Probe upload server reachability via uncached DNS + TCP connect.
...
// Probe upload server reachability using cached resolution path.
...
// Parse and validate an http:// URL into host/port/path parts.
...
// Build HTTP headers and multipart framing for one file upload request.
...
// Ensure one test payload file exists with expected size/content pattern.
bool ensure_single_test_file(const char* path) {
...
// Ensure all configured v5 test files exist on SD at boot.
...
// Classify HTTP status as retryable for transient server conditions.
