void configure_http_server() {
  ...
  s_server.on("/recover_sd", HTTP_GET, []() {
    const bool ok = recover_sd_safe("http/recover_sd");
  ...
  s_server.on("/test/recover_sd", HTTP_GET, []() {
    const bool ok = run_recover_sd_selftest();
    s_server.send(ok ? 200 : 500, "application/json", ok ? "{\"ok\":true}" : "{\"ok\":false}");
  });
...
void print_serial_help() {
  ...
  Serial.println("  recover_sd -> force SD recover routine");
  ...
  Serial.println("  test_recover_sd -> run SD recover self test");
    ...
    if (c == '\r' || c == '\n') {
      ...
      if (len > 0) {
        if (strcmp(cmd, "help") == 0) {
          ...
          const bool ok = recover_sd_safe("serial/recover_sd");
          Serial.println(ok ? "recover_sd PASS" : "recover_sd FAIL");
          ...
          const bool ok = run_recover_sd_selftest();
          Serial.println(ok ? "test_recover_sd PASS" : "test_recover_sd FAIL");
